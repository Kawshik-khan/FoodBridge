...
export async function PATCH(request: NextRequest) {
  ...
  const requestIndex = donorRequests.findIndex(req => req.id === parseInt(id!))
  if (requestIndex === -1) {
    return NextResponse.json({ error: 'Request not found' }, { status: 404 })
  }

  donorRequests[requestIndex] = { ...donorRequests[requestIndex], ...body }
  return NextResponse.json(donorRequests[requestIndex])
}